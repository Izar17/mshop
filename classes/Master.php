...
Class Master extends DBConnection {
	...
	function add_to_cart(){
		...
		$check = $this->conn->query("SELECT * FROM cart_list where product_id = '{$product_id}' && client_id = '{$client_id}' && attribute_id = '{$attribute_id}'")->num_rows;
		if($check > 0){
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$vendors = $this->conn->query("SELECT * FROM `vendor_list` where id in (SELECT vendor_id from product_list where id in (SELECT product_id FROM `cart_list` where id in ($selId) and client_id ='{$this->settings->userdata('id')}')) order by `shop_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $vendors->fetch_assoc()):
			...
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path, pa.amount FROM `cart_list` c 
				inner join product_list p on c.product_id = p.id 
                LEFT JOIN product_attribute pa ON c.attribute_id = pa.id
				where c.id in ($selId) and c.client_id = '{$this->settings->userdata('id')}' and p.vendor_id = '{$vrow['id']}' order by p.name asc");
				...
				if($save2){
					$this->conn->query("UPDATE `order_list` set `total_amount` = '{$gtotal}' where id = '{$oid}'");
					    // Deduct ordered quantity from product_list
					$products = $this->conn->query("SELECT product_id, quantity FROM `order_items` WHERE order_id = '{$oid}'");
					while($p = $products->fetch_assoc()){
						$this->conn->query("UPDATE `product_list` SET `quantity` = `quantity` - {$p['quantity']} WHERE id = '{$p['product_id']}'");
					}
				}else{
					$has_failed = true;
					$resp['sql'] = $sql2;
		...
		if(!$has_failed){
			...
			$this->conn->query("DELETE FROM `cart_list` where id in ($selId) and client_id ='{$this->settings->userdata('id')}'");
		}else{
			$resp['status'] = 'failed';
	...
	function confirm_order(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Order has been received successfully.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function accept_emergency(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `emergency` set `status` = 1 where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Emergency has been accept successfully.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function decline_emergency(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `emergency` set `status` = 2 where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Emergency has been declined!.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function fixed_emergency(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `emergency` set `status` = 3 where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Emergency has been fixed!.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function cancel_emergency(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `emergency` set `status` = 4 where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Emergency has been cancelled.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	...
	case 'accept_emergency':
		echo $Master->accept_emergency();
	break;
	case 'decline_emergency':
		echo $Master->decline_emergency();
	break;
	case 'cancel_emergency':
		echo $Master->cancel_emergency();
	break;
	case 'fixed_emergency':
		echo $Master->fixed_emergency();
	break;
	default:
		// echo $sysset->index();
		break;
